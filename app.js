...
let boxes = [];    // {id, x, y, z, w, d, h} — 파이프 아님. 참고용 반투명 육면체(공간 표시)
let hidden = new Set();   // 숨긴 오브젝트 키 'type:id' — 화면에서만 감춤(데이터·BOM엔 유지)
let special = { roof: 0, glasses: 0 };   // 자동 인식 불가 커넥터 수동 개수: 지붕·안경
let nextId = 1;
let activeLength = 20;
...
// 면의 두 변 길이(정렬)
// 판넬 규격 이름: 대(35×35 튜브칸) / 소(35×15 튜브칸) / 그 외는 이름 없음
function panelGrade(a, b) {
  const lo = Math.min(a, b), hi = Math.max(a, b);
  if (hi === 35 && lo === 35) return '대';
  if (hi === 35 && lo === 15) return '소';
  return '';
}
...
// ===== 사각형 프레임(파이프 4개) =====
// 가로 = activeLength, 세로/높이 = cubeHeight. 방향(rectDir)에 따라 놓는 평면이 달라진다.
function rectCorners(t) {
  const { ax, ay, az, sW, sH } = t;   // sW = 가로 간격, sH = 세로/높이 간격
  if (rectDir === 'floor') return [[ax, ay, az], [ax + sW, ay, az], [ax + sW, ay, az + sH], [ax, ay, az + sH]];
  if (rectDir === 'side')  return [[ax, ay, az], [ax, ay, az + sW], [ax, ay + sH, az + sW], [ax, ay + sH, az]];
  return [[ax, ay, az], [ax + sW, ay, az], [ax + sW, ay + sH, az], [ax, ay + sH, az]];   // front
}
// 사각형 배치 대상: 커서(부품 위면 그 지점, 아니면 바닥) → 격자 스냅. 바닥형은 스택, 벽형은 바닥(0)부터.
function rectGhostTarget() {
  const sW = activeLength + JOINT_SPAN, sH = cubeHeight + JOINT_SPAN;
  const hits = intersect(partsGroup.children);
  const pt = hits.length ? hits[0].point : groundPoint();
  if (!pt) return null;
  const ax = snapGrid(pt.x, sW), az = snapGrid(pt.z, sW);
  const ay = rectDir === 'floor' ? stackBaseY(ax, az, sW) : 0;
  return { ax, ay, az, sW, sH };
}
function showRectGhost(t) {
  clearGhost();
  const c = rectCorners(t).map(([x, y, z]) => new THREE.Vector3(x, y, z));
  ...
  const fill = new THREE.Mesh(geo, new THREE.MeshBasicMaterial({ color: 0x4aa3ff, transparent: true, opacity: 0.16, side: THREE.DoubleSide, depthWrite: false }));
  fill.add(new THREE.LineLoop(new THREE.BufferGeometry().setFromPoints([...c]), new THREE.LineBasicMaterial({ color: 0x8fcfff })));
  ghostGroup.add(fill);
  ghostGroup.visible = true;
}
// 사각형 4조인트·4파이프 추가 (화면 갱신은 호출부에서)
function stampRect(t) {
  const c = rectCorners(t).map(([x, y, z]) => findJointNear(x, y, z) || addJoint(x, y, z));
  const W = activeLength, H = cubeHeight;
  const axisOf = (p, q) => Math.abs(p.x - q.x) > 1 ? 'x' : Math.abs(p.y - q.y) > 1 ? 'y' : 'z';
  const edge = (a, b, len) => { if (!pipeBetween(c[a].id, c[b].id)) pipes.push({ id: uid(), a: c[a].id, b: c[b].id, length: len, axis: axisOf(c[a], c[b]) }); };
  edge(0, 1, W); edge(1, 2, H); edge(2, 3, W); edge(3, 0, H);
}
...
function rebuild() {
  ...
  for (const bx of boxes) {
    ...
    const mat = new THREE.MeshStandardMaterial({
      color: isSel ? 0x4aa3ff : 0xb98aff, emissive: isSel ? 0x1a4a80 : 0x000000,
      transparent: true, opacity: 0.22, depthWrite: false, side: THREE.DoubleSide, metalness: 0.05, roughness: 0.85,
    });
    const mesh = new THREE.Mesh(geo, mat);
    mesh.position.set(bx.x + bx.w / 2, bx.y + bx.h / 2, bx.z + bx.d / 2);
    mesh.add(new THREE.LineSegments(new THREE.EdgesGeometry(geo), new THREE.LineBasicMaterial({ color: isSel ? 0x8fcfff : 0x9a6cff })));
    mesh.userData = { type: 'box', id: bx.id };
    partsGroup.add(mesh);
  }
...
function deleteSelected() {
  ...
  const delPipes = del('pipe'), delJoints = del('joint'), delPanels = del('panel'), delBridges = del('bridge'), delSlides = del('slide'), delBoxes = del('box');
  if (delJoints.size) {
  ...
  if (delBoxes.size) boxes = boxes.filter(b => !delBoxes.has(b.id));
  pruneOrphanJoints();   // 파이프가 하나도 안 붙은 조인트는 함께 제거 (둥둥 뜬 조인트 방지)
  clearSel();
  rebuild();
  updateBOM();
  updateHideButton();   // 삭제로 사라진 숨김 키 정리
}
// ===== 숨김 / 표시 =====
function hideSelected() {
  if (selection.length === 0) { toast('숨길 대상을 먼저 선택하세요'); return; }
  for (const s of selection) hidden.add(s.type + ':' + s.id);
  clearSel(); activeJoint = null; handleGroup.visible = false; cubeGroup.visible = false;
  rebuild(); updateBOM(); updateHideButton();
  toast('선택 부품을 숨겼습니다');
}
function unhideAll() {
  ...
  hidden.clear();
  rebuild(); updateBOM(); updateHideButton();
  toast('숨긴 부품을 모두 표시했습니다');
}
function updateHideButton() {
  pruneHidden();
  const b = document.getElementById('btn-unhide');
  if (!b) return;
  b.textContent = `👁 숨김 해제 (${hidden.size})`;
  b.disabled = hidden.size === 0;
}
// 참고 박스 추가 (원점 바닥에 생성 → 선택 모드에서 드래그로 이동)
function addBox(w, d, h) {
  boxes.push({ id: uid(), x: 0, y: 0, z: 0, w, d, h });
  rebuild(); updateBOM();
}
// 사라진 오브젝트를 가리키는 숨김 키 정리
function pruneHidden() {
  for (const k of [...hidden]) {
    const i = k.indexOf(':');
    if (!memberExists({ type: k.slice(0, i), id: Number(k.slice(i + 1)) })) hidden.delete(k);
  }
}
// 연결된 파이프가 없는 조인트 제거
...
function memberExists(m) {
  return m.type === 'pipe' ? pipes.some(p => p.id === m.id)
    ...
    : m.type === 'slide' ? slides.some(s => s.id === m.id)
    : m.type === 'box' ? boxes.some(b => b.id === m.id) : false;
}
...
function boxSelect(x0, y0, x1, y1, additive) {
  ...
  for (const b of boxes) pick('box', b.id, new THREE.Vector3(b.x + b.w / 2, b.y + b.h / 2, b.z + b.d / 2));
  rebuild(); updateBOM();
}
...
function applyMove(e) {
  ...
  if (moving.kind === 'parts') {
    ...
    moving.box.x = moving.snap.x + dx; moving.box.z = moving.snap.z + dz;
    rebuild();
  } else {   // ruler
    const dx = Math.round((gp.x - moving.start.x) / JOINT_SPAN) * JOINT_SPAN;
    const dz = Math.round((gp.z - moving.start.z) / JOINT_SPAN) * JOINT_SPAN;
...
renderer.domElement.addEventListener('pointerdown', (e) => {
  ...
  if (mode === 'select' && e.button === 0) {
    setPointer(e);
    const gp = groundPoint();
    const partHit = intersect(partsGroup.children.filter(m => m.userData.type !== 'box'));
    const boxHit = intersect(partsGroup.children.filter(m => m.userData.type === 'box'));
    ...
    } else if (gp && boxHit.length && isSelected('box', boxHit[0].object.userData.id)) {   // 선택된 박스 드래그 → 박스 이동
      const b = boxes.find(x => x.id === boxHit[0].object.userData.id);
      if (b) moving = { kind: 'box', start: { x: gp.x, z: gp.z }, box: b, snap: { x: b.x, z: b.z } };
...
renderer.domElement.addEventListener('pointermove', (e) => {
  ...
  if (mode === 'select') {
    // 선택된 부품/박스/줄자 위면 이동 커서, 그 외는 선택 커서
    const ph = intersect(partsGroup.children.filter(m => m.userData.type !== 'box'));
    const bh = intersect(partsGroup.children.filter(m => m.userData.type === 'box'));
    const onSel = (ph.length && isSelected(ph[0].object.userData.type, ph[0].object.userData.id))
      || (bh.length && isSelected('box', bh[0].object.userData.id));
    renderer.domElement.style.cursor = (onSel || intersectRulers().length) ? 'move' : (ph.length || bh.length ? 'pointer' : 'crosshair');
    return;
  }
  ...
  // 사각형 모드: 바닥 스냅 고스트 (조인트 위에선 화살표/선택 우선 아님 — 항상 고스트)
  if (mode === 'rect') {
    const t = rectGhostTarget();
    if (t) { showRectGhost(t); renderer.domElement.style.cursor = 'copy'; return; }
    clearGhost(); ghostGroup.visible = false;
  }
...
function handlePointerUp(e) {
  ...
  // 3a) 선택 모드: 참고 박스 클릭 → 선택 (가장 낮은 우선순위 — 조인트/파이프/면 우선)
  if (mode === 'select') {
    const hitBox = intersect(partsGroup.children.filter(m => m.userData.type === 'box'));
    if (hitBox.length) {
      const id = hitBox[0].object.userData.id;
      if (e.shiftKey) toggleSel('box', id); else selectOne('box', id);
      activeJoint = null; handleGroup.visible = false; cubeGroup.visible = false;
      rebuild(); updateBOM();
      return;
    }
  ...
  // 3c) 사각형 모드: 커서 위치에 파이프 4개 프레임 배치
  if (mode === 'rect') {
    const t = rectGhostTarget();
    if (t) {
      stampRect(t);
      clearSel(); activeJoint = null; clearGhost(); ghostGroup.visible = false;
      rebuild(); updateBOM();
      return;
    }
  }
...
// 방향들이 걸친 축(X/Y/Z) 개수 — 4방향이 2축이면 한 평면(+자), 3축이면 입체(4구)
function axisSpan(dirs) {
  const ax = new Set();
  for (const d of dirs) {
    if (Math.abs(d.x) > 0.9) ax.add('x');
    else if (Math.abs(d.y) > 0.9) ax.add('y');
    else if (Math.abs(d.z) > 0.9) ax.add('z');
  }
  return ax.size;
}
...
function classifyJoint(jointId) {
  ...
  if (deg === 1) return '1구';                                          // 끝(1구)
  if (deg === 2) return dirs[0].dot(dirs[1]) < -0.9 ? '1자' : '2구';    // 일직선(1자) / ㄱ자 꺾임(2구)
  if (deg === 3) return hasColinearPair(dirs) ? 'T자' : '3구';           // 평면 T / 직각 코너
  if (deg === 4) return axisSpan(dirs) === 2 ? '+자' : '4구';            // 평면 십자(+자) / 입체 4구
  return deg + '구';                              // 5구/6구
}
...
function computeBOM() {
  ...
  for (const panel of panels) {
    ...
    const base = panel.hole ? '구멍면' : '판넬';
    const grade = panelGrade(a, b);   // 대(35×35) / 소(35×15) / 그 외 없음
    const label = `${base}${grade ? ' ' + grade : ''} ${a}×${b}cm`;
...
// 특수 커넥터(지붕·안경) 수동 개수 표시 갱신
function updateSpecialUI() {
  const r = document.getElementById('sp-roof'), g = document.getElementById('sp-glasses');
  if (r) r.textContent = special.roof;
  if (g) g.textContent = special.glasses;
}
...
function updateBOM() {
  ...
  const JOINT_ORDER = ['1구', '1자', '2구', '3구', '4구', '5구', '6구', 'T자', '+자'];
  ...
  // 특수 커넥터(지붕·안경) — 수동 입력분
  const specialEl = document.getElementById('bom-special');
  const specialList = [['지붕', special.roof], ['안경', special.glasses]].filter(([, n]) => n > 0);
  let totalSpecial = 0, specialTypes = 0;
  if (specialList.length) {
    let xhtml = '<div class="bom-sub">특수 커넥터</div>';
    for (const [name, n] of specialList) {
      totalSpecial += n; specialTypes++;
      xhtml += `<div class="bom-item"><span>${name}</span><span class="qty">${n}</span></div>`;
    }
    specialEl.innerHTML = xhtml;
  } else specialEl.innerHTML = '';

  // 참고 박스(파이프 아님 — 부품 수엔 미포함)
  const boxEl = document.getElementById('bom-boxes');
  if (boxes.length) {
    const cnt = {};
    for (const b of boxes) { const k = `${b.w}×${b.d}×${b.h}cm`; cnt[k] = (cnt[k] || 0) + 1; }
    let bhtml = '<div class="bom-sub">참고 박스 <span class="bom-hint">부품 아님</span></div>';
    for (const k of Object.keys(cnt).sort()) bhtml += `<div class="bom-item"><span>박스 ${k}</span><span class="qty">${cnt[k]}</span></div>`;
    boxEl.innerHTML = bhtml;
  } else boxEl.innerHTML = '';

  // 합계 (총 수 + 종류 수)
  document.getElementById('bom-total').innerHTML =
    `<div class="total-line"><span>총 부품 수</span><span>${totalPipes + totalJoints + totalPanels + totalBridges + totalSlides + totalSpecial}개</span></div>` +
    `<div class="total-line sub"><span>부품 종류</span><span>${pipeTypes + jointTypes + panelTypes + bridgeTypes + slideTypes + specialTypes}종류</span></div>` +
...
function currentDesign() {
  return { joints, pipes, panels, bridges, slides, boxes, notes, rulers, groups, special, hidden: [...hidden], activeLength, nextId };
}
...
function applyLoaded(d) {
  ...
  rulers = d.rulers || [];
  groups = d.groups || [];
  special = { roof: 0, glasses: 0, ...(d.special || {}) };
  hidden = new Set(d.hidden || []);
  activeLength = d.activeLength || 20;
  nextId = d.nextId || (Math.max(0, ...joints.map(j => j.id), ...pipes.map(p => p.id), ...panels.map(p => p.id), ...bridges.map(b => b.id), ...slides.map(s => s.id), ...boxes.map(b => b.id), ...notes.map(n => n.id), ...rulers.map(r => r.id)) + 1);
  clearSel(); activeJoint = null; handleGroup.visible = false; cubeGroup.visible = false;
  buildPipeButtons(); rebuild(); updateBOM(); renderNotes(); rebuildRulers(); updateSpecialUI(); updateHideButton();
}
...
function copyBOM() {
  ...
  txt += '\n조인트\n';
  for (const t of ['1구', '1자', '2구', '3구', '4구', '5구', '6구', 'T자', '+자'])
  ...
  const specList = [['지붕', special.roof], ['안경', special.glasses]].filter(([, n]) => n > 0);
  if (specList.length) {
    txt += '\n특수 커넥터\n';
    for (const [name, n] of specList) { txt += `  ${name} x ${n}\n`; total += n; types++; }
  }
...
function clearDesign() {
  joints = []; pipes = []; panels = []; bridges = []; slides = []; boxes = []; notes = []; groups = []; special = { roof: 0, glasses: 0 }; hidden = new Set(); nextId = 1; selection = []; activeJoint = null;
  handleGroup.visible = false; cubeGroup.visible = false;
  renderNotes(); updateSpecialUI(); updateHideButton();
}
function resetAll() {
...
document.querySelectorAll('.mode-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.getElementById('len-hint').textContent =
      ...
      : mode === 'rect' ? '파이프 4개로 된 사각형 프레임을 놓습니다. 방향(바닥/앞벽/옆벽)을 고르고, 바닥을 클릭하세요. 가로=파이프 길이, 세로=육면체 높이.'
    ...
    document.getElementById('rect-section').style.display = mode === 'rect' ? 'block' : 'none';
...
document.getElementById('btn-add-box').addEventListener('click', () => {
  ...
  withUndo(() => addBox(w, d, h));
  toast('반투명 박스 추가 — 선택 모드에서 드래그해 옮기세요');
});

document.getElementById('btn-delete').addEventListener('click', () => withUndo(deleteSelected));
document.getElementById('btn-hide').addEventListener('click', () => withUndo(hideSelected));
document.getElementById('btn-unhide').addEventListener('click', () => withUndo(unhideAll));
...
document.querySelectorAll('.sp-btn').forEach(btn => btn.addEventListener('click', () => {
  const k = btn.dataset.sp, delta = parseInt(btn.dataset.d, 10);
  withUndo(() => { special[k] = Math.max(0, (special[k] || 0) + delta); });
  updateSpecialUI(); updateBOM();
}));
...
// 사각형 방향 버튼
document.querySelectorAll('.rect-dir-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    rectDir = btn.dataset.rdir;
    document.querySelectorAll('.rect-dir-btn').forEach(b => b.classList.toggle('active', b === btn));
    clearGhost(); ghostGroup.visible = false;   // 방향 바뀌면 고스트 갱신 (다음 이동에서 재생성)
  });
});
